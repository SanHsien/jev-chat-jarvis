...
| `app/src/main/java/com/jev/probe/overlay/OverlayController.kt` | 兩處註解提到微信。 | 改為通用敘述；新增 `showPending()`（半自動：分析對象 + 「分析」「略過」）。 | 同上；維護者要求省 token。 | 照收上游，保留 `showPending()`。 |
