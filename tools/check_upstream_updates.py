...
``release`` (default here)
    ...
    granularity -- each upstream release (v1.3, v1.4, ...) lands as one
    reviewed batch -- while upstream `main` moves several times a day. Tracking the
...
def parse_tag_refs(raw: str) -> list[tuple[tuple[int, int, int], str, str]]:
    ...
    for name, sha in commits.items():
        ...
        if match is None:  # pragma: no cover - filtered above
            continue
        parsed.append((tuple(int(part or 0) for part in match.groups()), name, sha))
...
def history_rewritten(reviewed: str, upstream_ref: str, repo_dir: Path) -> bool:
    """True when the reviewed commit is no longer part of upstream's history.

    A force-push that rewrites upstream history leaves ``reviewed_through``
    unreachable from upstream's branch; ``reviewed..ref`` then lists upstream's
    entire history as "new", which reads like a huge review queue instead of the
    one decision actually needed (re-anchor the baseline).
    """
    result = subprocess.run(
        ["git", "merge-base", "--is-ancestor", reviewed, upstream_ref],
        cwd=repo_dir,
        capture_output=True,
        text=True,
    )
...
def main() -> int:
    ...
    try:
        ...
        branch_ref = fetch_upstream(baseline, args.repo_dir)
        if history_rewritten(baseline["reviewed_through"], branch_ref, args.repo_dir):
            raise UpstreamCheckError(
                f"upstream rewrote its history: {baseline['reviewed_through'][:7]} is no longer "
                f"reachable from upstream {baseline['branch']}. Find the upstream commit with "
                "the same tree, merge it with `git merge -s ours --allow-unrelated-histories`, "
                "and move reviewed_through there (see docs/UPSTREAM.md)."
            )
