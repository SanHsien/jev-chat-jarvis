...
def test_a_rewritten_upstream_history_is_detected(tmp_path: Path) -> None:
    """After an upstream force-push the reviewed commit is unreachable upstream."""
    repo = tmp_path / "r"
    repo.mkdir()
    ...
    _git(["add", "."], repo)
    _git(["commit", "-m", "old"], repo)
    old = _git(["rev-parse", "HEAD"], repo)
    _git(["checkout", "--orphan", "rewritten"], repo)
    _git(["commit", "-m", "new root"], repo)
    new = _git(["rev-parse", "HEAD"], repo)
    _git(["commit", "--allow-empty", "-m", "on top"], repo)

    assert checker.history_rewritten(old, "rewritten", repo) is True
    assert checker.history_rewritten(new, "rewritten", repo) is False
