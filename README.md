...
```bash
adb install -r jev-assistant-v1.4.10.apk   # debug 簽章的版本檔名結尾是 -debug.apk
```
...
### 半自動分析（預設）
- 偵測到對方的新訊息時，懸浮窗只顯示**分析對象**（群組裡是最新發言者）與最新一則訊息，按「分析」才送出判斷與起草，按「略過」收起。
- **為什麼**：貼圖、照片、「好」「哈哈」這類訊息多半不需要分析，每則都自動送出會白花 token。想要每則都自動分析，到設定開「對方發訊息時自動分析」。
...
### 使用方式
- 對方來新訊息 → 懸浮窗給判斷摘要（建議動作、可能意圖、對方需要、緊張度 0–9）與三條候選（附 Jev 勝出機率）→「填入」→ **你自己看過、改過再按發送**。
...
`versionName` 為 `上游主.次.fork 序號`（目前 `1.4.10`，基於上游 1.4）；變更紀錄見 [`CHANGELOG.md`](CHANGELOG.md)。
