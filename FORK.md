...
沿用 MIT License（`LICENSE`、`NOTICE`）。`main` 是單一根 commit，不含上游的 Git 歷史；上游基準以 SHA 記在 `tools/upstream_baseline.json`（Windows 版見 `docs/DIVERGENCE-windows.md`），檢查工具需要時會自動向上游 fetch。

## 為什麼維護 fork

- 追蹤與使用上游 Jev 聊天助手，發行為「對話副駕」（Android 非侵入式對話副駕：無障礙讀畫面 → Jev 判斷 → 候選回覆 → 人工發送）。
- **支援 LINE（本 fork 原創，上游沒有）**：維護者的主要聊天軟體。`LineAdapter` 自動讀取已用真機 dump 驗證（2026-09-25）。
- **移除微信、飛書與 QQ**：程式碼與說明皆已刪除。
- **Windows 版（`wingman/`）**：jev-chat-windows 拆進本 repo 結構（套件、依賴、打包、授權），改為只認 LINE 電腦版，見 [`docs/DIVERGENCE-windows.md`](docs/DIVERGENCE-windows.md)。
- 判斷接口改以 Vercel AI Gateway 為預設（次選 OpenRouter）。
- 接口預設只保留 OpenRouter、Vercel、TypeSafe、OpenCode Zen 與自定義。
...
| 項目 | 說明 |
|---|---|
| `AGENTS.md` / `CLAUDE.md` | AI 維護規則與產品硬約束的單一真相源；`CLAUDE.md` 只匯入 `AGENTS.md` |
| `NOTICE.md` / `FORK.md` | 來源、授權與同步說明 |
| `README.md` / `README.en.md`、`CHANGELOG.md` / `CHANGELOG.en.md` | 繁中主檔與內容一致的英文鏡像 |
| `SECURITY.md` / `CONTRIBUTING.md` / `CODE_OF_CONDUCT.md` | 回報與協作規則 |
| `REVIEW.md` | 證據式倉庫審查（只留最新一輪） |
...
| `.github/workflows/release.yml` | 建置 APK 並發佈到 GitHub Releases，之後呼叫 `repo-cleanup.yml` |
| `.github/workflows/repo-cleanup.yml` | 只留 `main`、最新 release 與其 tag |
...
| `.github/ISSUE_TEMPLATE/config.yml`、`.github/pull_request_template.md` | issue 分流（本 fork／上游／安全）與 PR 驗證清單 |
...
產品程式碼的改動（LINE 適配器、移除微信／飛書／QQ、Vercel 預設、AGP 9 升級）逐檔登記在 [`docs/DIVERGENCE.md`](docs/DIVERGENCE.md)。
