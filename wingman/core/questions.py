...
# choice 類答案的中文說法，介面和起草小抄共用這一份（app/overlay.py 從這裡導）。
CHOICE_LABELS: dict = {
    "true_intent": {
        "confirm_you_care": "希望確認你在意", "vent_anger": "表達不滿或受傷",
        "request_action": "希望你採取行動", "seek_explanation": "希望瞭解原因",
        "casual_chat": "輕鬆交流", "close_topic": "平和結束話題",
    },
    "best_action": {
        "check_history": "先核對聊天記錄", "apologize": "為已知問題道歉",
        "give_commitment": "給出具體承諾", "explain": "說明事實與原因",
        "acknowledge": "回應並表達理解", "say_less": "簡短回應或留白",
        "make_plan": "商量具體安排",
    },
    "she_needs": {
        "apology": "真誠道歉", "action": "具體行動或安排", "explanation": "清楚的解釋",
        "care": "關注與在意", "nothing": "可能無需補充回應",
    },
}

_GUIDE_FIELDS = (("true_intent", "對方意圖"), ("she_needs", "對方需要"),
                 ("best_action", "建議動作"))


def guidance_text(answers: dict) -> str:
    """Jev 判斷 → 餵給起草的中文小抄。只寫有答案的那幾項；沒答案返回空串。"""
    lines = []
    ...
    if isinstance(score, (int, float)) and not isinstance(score, bool):
        tail.append(f"緊張度：{score:.0f}/9")
    noul = ((answers or {}).get("literal_question") or {}).get("noul")
    if isinstance(noul, (int, float)) and not isinstance(noul, bool):
        tail.append("字面意思：" + ("是" if noul >= 0.5 else "否（有潛臺詞）"))
    if tail:
        lines.append("- " + "；".join(tail))
    if not lines:
        return ""
    return "判斷參考（Jev 給的，起草要順著它寫，但口吻仍按我的）：\n" + "\n".join(lines)


def build_state(messages: list, relationship: str, keep: int = 10,
                reply_to: str | None = None) -> dict:
    """messages: (from, text) / (from, text, name) / dict（name 可選）。from 只認 her/me。

    name = 群裡的發言人；有 name 就當群聊（chat.is_group）。reply_to = 群裡指定的回覆對象。
    """
    cleaned = []
