 ...
 * settings ("清空知識庫與歷史"), which deletes that directory and nothing else.
 */
 ...
 * LINE / X, and each of those titles can be listed here.
 */
data class Contact(
    val id: String,
    val name: String,
    val aliases: List<String> = emptyList(),
    /** Package names this contact has been seen in, e.g. jp.naver.line.android. */
...
data class ChatContext(
     ...
     *        A contact with no relationship set simply omits the "關係：" line.
     */
    fun background(defaultRelationship: String): String {
        val sb = StringBuilder()
        contact?.let { c ->
            val rel = c.relationship.trim()
            if (rel.isNotEmpty()) sb.append("關係：").append(rel).append('\n')
            if (c.notes.isNotBlank()) sb.append("關於").append(c.name).append("：")
                .append(c.notes.trim()).append('\n')
            if (c.autoSummary.isNotBlank()) sb.append("過往摘要：")
