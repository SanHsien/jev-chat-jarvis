...
import com.jev.probe.core.otherSpeakers
...
object JevQuestions {
    ...
    fun buildState(
        ...
        // Group chats (fork): each line carries the speaker's name and the chat names
        // the person to judge. A 1:1 chat sends exactly the calibrated body as before.
        val group = snapshot.isGroupChat()
        val msgs = JSONArray()
        val last10 = snapshot.messages.takeLast(10)
        for (m in last10) {
            val o = JSONObject().put("from", m.side).put("text", m.text)
            if (group && m.speaker != null) o.put("name", m.speaker)
            msgs.put(o)
        }
        val chat = JSONObject()
        ...
        if (group && focus != null) {
            chat.put("type", "group")
                .put("participants", JSONArray(snapshot.otherSpeakers()))
                .put("focus_person", focus)
                .put("note", "Group chat. \"The other person\" means focus_person, the sender of the " +
                    "latest message from the other side; other participants' messages are context only.")
        }
