...
class ScreenCapture(
    ...
    fun capture(onResult: (Result) -> Unit) {
        ...
        if (now - lastAttemptAt < need) {
            onResult(Result.Failed(CODE_THROTTLED, "截圖太頻繁"))
            return
        }
        lastAttemptAt = now
    ...
    private fun shoot(finish: (Result) -> Unit, done: AtomicBoolean) {
        ...
        try {
            ...
            finish(Result.Failed(CODE_INTERNAL, "截圖失敗：${e.javaClass.simpleName}"))
        }
    }
    ...
    private fun toBitmap(result: AccessibilityService.ScreenshotResult, window: Rect?): Result {
        ...
        return try {
            ...
            if (bmp == null) {
                Result.Failed(CODE_INTERNAL, "截圖失敗：拿到的畫面讀不出來")
            } else {
            ...
            Result.Failed(CODE_INTERNAL, "截圖失敗：${e.javaClass.simpleName}")
        } finally {
    ...
    companion object {
        ...
        fun humanMessage(code: Int): String = when (code) {
            CODE_THROTTLED -> "截圖太頻繁"
            CODE_TIMEOUT -> "截圖超時"
            1 -> "截圖失敗：內部錯誤（系統拒絕，可能是該無障礙服務不被允許截圖）"
            2 -> "截圖失敗：無障礙服務未宣告截圖能力（去設定裡把無障礙關掉再開啟）"
            3 -> "截圖失敗：間隔太短，等一秒再試"
            4 -> "截圖失敗：沒有有效的顯示"
            6 -> "截圖失敗：視窗不可見或受保護（FLAG_SECURE），這類介面拿不到畫面"
            else -> "截圖失敗（碼 ${code}）"
        }
    }
}
